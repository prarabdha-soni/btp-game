//Prarabdha soni Roll no. 130101058
...
void loadImages(void) {
	...
	for(int i = 0; i < 8; i++) {
		...
		if(fp == NULL) {
			...
			switch(i) {
				case 0:	dino1 = loadArray(fp, DW, DH); break;
				case 1:	dino2 = loadArray(fp, DW, DH); break;
				case 2:	dino3 = loadArray(fp, DW, DH); break;
				case 3:	dino4 = loadArray(fp, DW, DH); break;
				case 4:	dino5 = loadArray(fp, DW, DH); break;
				case 5:	dino6 = loadArray(fp, DW, DH); break;
				case 6: cacti = loadArray(fp, CW, CH); break;
				case 7: cloud = loadArray(fp, LW, LH); break;
			}
		}
	}
}
...
void reset(void) {
	runtime = 0; halt = false; started = true; score = 0;
	dinoState = 0; dinoHS = 0;	
...
void loop(int val) {
	...
	if(halt == true)	goto skip;

	if(runtime % dinoPeriod  == 0)	updateDino();
	if(runtime % cactiPeriod == 0)	updateCacti();
	checkCollision();

	if(runtime % (10 * cactiPeriod) == 0) score++;

	glutPostRedisplay();

	skip:	glutTimerFunc(refreshPeriod, loop, 0);
}
...
void updateDino(void) {
	...
	if(dinoJumpEnable == true) {
		dinoHS = ((dinoHS + 1) % 19);
		if(dinoHS == 0)	dinoJumpEnable = false;
	}
}

void updateCacti(void) {
	if(cactiPos[4] == winRightEdge)	cactiLastPushed = runtime;
...
void drawFigure(int f, int xpos, int ypos, int yoff, int w, int h) {
	...
	switch(f) {
		case 0: temp = dino1; break;
		case 1: temp = dino2; break;
		case 2: temp = dino3; break;
		case 3: temp = dino4; break;
		case 4: temp = dino5; break;
		case 5: temp = dino6; break;
		case 6: temp = cacti; break;
		case 7: temp = cloud; break;
	}
...
char *intToStr(int n) {
	...
	for(int i = 0; i < k; i++)	tem[i] = s[k - i - 1];
	tem[k] = '\0';
